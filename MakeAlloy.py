		...
		if (index > -1):
			if (self.panel[index] == "clean"):
				...
				if (self.panel[index] == "compile"):
					self.window.run_command("exec", {"cmd": [self.alloy, "-t", sdk.strip(), "compile", self.root]})
				else:
					platform = self.panel[index].split(' ')
					self.window.run_command("exec", {"cmd": [self.alloy, "-t", sdk.strip(), "run", self.root, platform[1]]})

	def _input_panel_callback(self, text):
		cmd = self.generate.split(' ')
		self.window.run_command("exec", {"cmd": [self.alloy, "-o", self.root + "/app", "generate", cmd[2], text]})
